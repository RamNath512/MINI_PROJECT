...
void login()
{
    ...
    char user[10]="ramnath";	//Predifined username and pass word
    char pass[10]="sriram";
	//system("color 0");
    do
	{
		...
		pword[i]='\0';
		i=0;
		if(strcmp(uname,"ramnath")==0 && strcmp(pword,"sriram")==0)
		{
			printf("  \n\n\nWELCOME TO C PROGRAM STUDENT FEE PAYMENT AND EMPLOYEE PAYROLL MANAGEMENT SYSTEM PROJECT !!!! LOGIN IS SUCCESSFUL");
    ...
    if (j==1)
    {
        while(c=='y'||c=='Y')
        {
            ...
            else if (choice==3)
            {
                ...
                while(fread(&stud,sizeof(stud),1,fs)==1)
                {
                    if(stud.r==rll) 					//(strcmpi(name,stud.n)==0)
                    {
                        a=0;
                        printf("\nname = %s",stud.n);
                        printf("\nclass = %d",stud.c);
...
void ext()
{
    int i;
    system("color 0c");
    printf("\n\n\t\t Thanks for using our STUDENT FEE PAYMENT AND EMPLOYEE PAYROLL MANAGEMENT SYSTEM PROJECT. .\n\n");
    system("pause");
    system("cls");
    printf("\n\n\t\t\t Exiting In A Moment\n\n");
    for(i=1; i<=60; i++)
...
int main()
{
	...
	printf("\t	   ***********STUDENT FEE PAYMENT AND *************    \n"); 
	printf("\t     ***EMPLOYEE PAYROLL MANAGEMENT SYSTEM PROJECT***    \n");
    printf("\t           $-DEVELOPED BY RAM NATH AND SRI RAM-$         \n");
    for(i=0;i<99;i++)
	{
		printf("-");
	}
    ...
    printf("\n\n\t** WELCOME TO C PROGRAM STUDENT FEE PAYMENT AND EMPLOYEE PAYROLL MANAGEMENT SYSTEM PROJECT **\n\n\n");
    for(i=0; i<80; i++)
    {
        printf("\xdb");
    }
